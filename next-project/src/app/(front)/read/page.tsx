...
import { getUserEmail, getSession } from "@/app/api/auth/sessionCheck/route";
	...
	if(!!user) {
		for(const place of places){
			addUserToPlace({placeId: place.id, userId: user.id}).then(() => {
				if(!!user && (user?.places.length + 1 >= clearNum)) {console.log("クリア！"); updateUserCleared(user.id, clearNum)};
			});
		}
	}
